...
class PatientService:
    ...
    def list_patients(
        self, db: Session, *, page: int = 1, size: int = 20, search: Optional[str] = None
    ) -> Tuple[List[Patient], int]:
        """Fetch list of patients with calculated pagination and search filters."""
        if page < 1:
            page = 1
        if size < 1:
            size = 20
        skip = (page - 1) * size
        items = patient_repository.list_patients(db, skip=skip, limit=size, search=search)
        total = patient_repository.count_patients(db, search=search)
        return items, total

    def create_patient(self, db: Session, patient_in: PatientCreate) -> Patient:
        """Create a new unified patient record (demographics + sub-records)."""
    ...
    def delete_patient(self, db: Session, patient_id: uuid.UUID) -> Patient:
        """Delete a patient record and cascade drop relationships."""
        patient = patient_repository.get_by_id(db, patient_id)
        if not patient:
            raise ValueError("Patient not found")
